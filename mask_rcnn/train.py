...
from my_dataset_coco import MGDetection
...
def main(args):
    ...
    # load train data set
    # coco2017 -> annotations -> instances_train.json
    train_dataset = MGDetection(data_root, "train", data_transform["train"])
    # VOCdevkit -> VOC2012 -> ImageSets -> Main -> train.txt
    # train_dataset = my_dataset_coco(data_root, year="2012", txt_name="train.txt", transforms=data_transform["train"])
    train_sampler = None
    ...
    val_dataset = MGDetection(data_root, "val", data_transform["val"])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_best is True:
            torch.save(save_files, "save_weights/" + current_date + "_best_model.pth")
        else:
            torch.save(save_files, "save_weights/" + current_date + "_model_{}.pth".format(epoch))
...
def parse_args():
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='./data/mgs', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=2, type=int, help='num_classes')  # VOC不包含背景只有20类
    # 文件保存地址
