...
class VOCInstances(Dataset):
    def __init__(self, mg_root, year="2012", txt_name: str = "train.txt", transforms=None):
        super().__init__()
        if isinstance(year, int):
            year = str(year)
        assert year in ["2007", "2012"], "year must be in ['2007', '2012']"
        if "VOCdevkit" in mg_root:
            root = os.path.join(mg_root, f"VOC{year}")
        else:
            root = os.path.join(mg_root, "VOCdevkit", f"VOC{year}")
        ...
        # read class_indict
        json_file = 'indices_json/pascal_voc_indices.json'
...
if __name__ == '__main__':
    dataset = VOCInstances(mg_root="/data/")
    print(len(dataset))
    d1 = dataset[0]
