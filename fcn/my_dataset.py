...
class MGSegmentation(data.Dataset):
    def __init__(self, root, transforms=None, txt_name: str = "train.txt"):
        super(MGSegmentation, self).__init__()
        assert os.path.exists(root), "path '{}' does not exist.".format(root)
        image_dir = os.path.join(root, 'JPEGImages')
        mask_dir = os.path.join(root, 'SegmentationClass')
        ...
        self.masks = [mask_dir + '/' + x + "c.png" for x in file_names]
        assert (len(self.images) == len(self.masks))
        self.transforms = transforms
...
# dataset = MGSegmentation("D:/myProjects/datasets/mg_voc_cm/")
